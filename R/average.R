...
#' fuzzy.matchable.x <- lapply(list(letters[1:4], letters[1:3]), function(x) paste0("var ", x))
#' fuzzy.matchable.y <- lapply(list(LETTERS[5:1], LETTERS[4:1]), function(x) paste0("var ", x))
#' x <- matrix(1:12, nrow = 4, ncol = 3, dimnames = fuzzy.matchable.x)
#' y <- matrix(1:20, nrow = 5, ncol = 4, dimnames = fuzzy.matchable.y)
