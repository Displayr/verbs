...
namesAreNotFuzzyMatchable <- function(names.to.match) {
    ...
    is.date.time <- lapply(names.to.match, .isDateTime)
    either.cond <- mapply(`|`, below.min.nchar, is.date.time, SIMPLIFY = FALSE)
    outer(either.cond[[1L]], either.cond[[2L]], FUN = `|`)
}
...
findLevenshteinMatches <- function(names.to.match, mapping.list)
{
    ...
    wont.match <- namesAreNotFuzzyMatchable(names.to.match)
    name.distances[wont.match] <- Inf
...
fuzzyMatchDimensionNames <- function(x.names, hide.unmatched, warn = TRUE)
{
    ...
    if (!none.match)
    {
        ...
        unmatched.names <- lapply(x.names, setdiff, y = exact.matches)
    } else
    {
        exact.matched.indices <- replicate(2, NULL)
        unmatched.names <- x.names
    }
    if (unmatchedIsEmpty(unmatched.names))
...
unmatchedIsEmpty <- function(x) identical(unlist(x), character(0L)) || is.null(x)
