...
quoted.function <- sQuote("SumEachRow")
...
test_that("Variables", {
    ...
    expect_error(SumEachRow(variable.Text), text.error, fixed = TRUE)
    datetime.error <- capture_error(throwErrorInvalidDataForNumericFunc("Date/Time", quoted.function))[["message"]]
    expect_error(SumEachRow(variable.Date), datetime.error)
    numeric.var.expected <- as.vector(variable.Numeric)
    numeric.var.expected.wo.missing <- numeric.var.expected
    expect_equal(SumEachRow(variable.Numeric, remove.missing = FALSE), numeric.var.expected)
    expect_equal(SumEachRow(variable.Numeric, remove.missing = TRUE), numeric.var.expected.wo.missing)
    ...
    expect_equal(SumEachRow(variable.Nominal, remove.missing = FALSE), nominal.var.expected)
    expect_equal(SumEachRow(variable.Nominal, remove.missing = TRUE), nominal.var.expected.wo.missing)
    binary.var.expected <- as.vector(variable.Binary)
    binary.var.expected.wo.missing <- binary.var.expected
    expect_equal(SumEachRow(variable.Binary, remove.missing = FALSE), binary.var.expected)
    expect_equal(SumEachRow(variable.Binary, remove.missing = TRUE), binary.var.expected.wo.missing)
    df <- data.frame(variable.Numeric, variable.Date)
    expect_error(SumEachRow(df), datetime.error)
    df <- data.frame(variable.Binary, variable.Text)
    expect_error(SumEachRow(df), text.error)
    ...
    expected.2.sum.rows.missing.kept <- setNames(rowSums(vars.in.df[-3]),
                                                 1:nrow(vars.in.df))
    expect_equal(SumEachRow(vars.in.df[-3], remove.missing = TRUE),
                 expected.2.sum.rows.missing.removed)
    expect_equal(SumEachRow(vars.in.df[-3], remove.missing = FALSE),
    ...
    expected.3.sum.rows.missing.kept <- setNames(rowSumsNaAdjusted(vars.in.df, remove.missing = FALSE),
                                                 1:nrow(vars.in.df))
    expect_equal(SumEachRow(vars.in.df, remove.missing = TRUE),
                 expected.3.sum.rows.missing.removed)
    expect_equal(SumEachRow(vars.in.df, remove.missing = FALSE),
    ...
    captured.warnings <- capture_condition(SumEachRow(df, warn = TRUE))
    ...
    expect_warning(SumEachRow(data.frame(1:5, factor(1:5)), warn = TRUE),
...
test_that("Table 1D", {
    expect_equivalent(SumEachRow(table1D.Percentage), table1D.Percentage)
    expect_equal(SumEachRow(table.1D.MultipleStatistics), rowSums(table.1D.MultipleStatistics))
    expected.output <- rowSums(table.1D.MultipleStatistics[, -which(colnames(table.1D.MultipleStatistics) == "z-Statistic")])
    expect_equal(SumEachRow(table.1D.MultipleStatistics,
    ...
    captured.warnings <- capture_warnings(SumEachRow(table.1D.MultipleStatistics,
                                                     remove.columns = "z-Statistic",
                                                     warn = TRUE))
...
test_that("Table 2D", {
    ...
    expect_equal(SumEachRow(table2D.Percentage), expected.2d.row.sums)
    expected.2d.row.sums <- rowSums(table2D.PercentageNaN[, -10], na.rm = TRUE)
    expect_equal(SumEachRow(table2D.PercentageNaN), expected.2d.row.sums)
    ...
    expect_equal(SumEachRow(table2D.PercentageAndCount),
    ...
    expect_condition(SumEachRow(table2D.PercentageNaN, warn = TRUE), missing.value.warning)
    # Missing values
    expect_true(anyNA(SumEachRow(table2D.PercentageNaN, remove.missing = FALSE)))
    expect_false(anyNA(SumEachRow(table2D.PercentageNaN)))
    ...
    expect_equal(SumEachRow(subsetted.qtable),
    ...
    captured.warnings <- capture_warnings(expect_equal(SumEachRow(table.opp.inf, warn = TRUE),
                                                       expected.out))
...
test_that("Higher dim Q tables", {
    ...
    expect_equal(SumEachRow(curr.table),
...
test_that("A single R Output (e.g. a vanilla matrix or vector) selected", {
    ...
    expect_equal(SumEachRow(matrix.1), rowSums(matrix.1))
    vector.1 <-1:24
    expect_equal(SumEachRow(vector.1), vector.1)
    ...
    expect_error(SumEachRow(array.1), higher.dim.array.error)
})
...
test_that("Column names conflicting with function argument names won't cause an error", {
    ...
    expect_equal(SumEachRow(input), rowSums(input))
})

test_that("SumEachRow alias working", {
    expect_equal(SumEachRow, SumEachRow)
    expect_equal(SumEachRow(table2D.Percentage),
                 SumEachRow(table2D.Percentage))
})

test_that("Handling of NAs", {
    expect_equal(SumEachRow(array(NA, dim = c(5L, 1L)), remove.missing = TRUE, warn = FALSE),
                 rep(NA_integer_, 5L))
    expect_equal(SumEachRow(NA, remove.missing = FALSE),
                 NA_integer_)
    expect_equal(SumEachRow(NA, remove.missing = TRUE),
                 NA_integer_)
    expect_equal(SumEachRow(array(c(1:3, NA), dim = c(4L, 1L))), c(1:3, NA))
    n <- 10L
    ...
    expect_equal(SumEachRow(dat.with.missing),
                 setNames(rowSumsNaAdjusted(dat.with.missing, remove.missing = TRUE),
                          1:n))
    expect_equal(SumEachRow(dat.with.missing, remove.missing = FALSE),
...
test_that("Warnings muffled", {
    ...
    expect_equal(SumEachRow(input.array, warn = "Foo"), rowSums(input.array[, -1L]))
})
