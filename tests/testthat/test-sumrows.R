...
test_that("Variables", {
    expect_error(SumForEachRow(variable.Text),
                 paste0("Text data has been supplied but ", sQuote("SumForEachRow"), " requires numeric data."))
    expect_error(SumForEachRow(variable.Date),
                 paste0("Date/Time data has been supplied but ", sQuote("SumForEachRow"), " requires numeric data."))
    ...
    expect_equal(SumForEachRow(variable.Numeric, remove.missing = FALSE), numeric.var.expected)
    expect_equal(SumForEachRow(variable.Numeric, remove.missing = TRUE), numeric.var.expected.wo.missing)
    ...
    expect_equal(SumForEachRow(variable.Nominal, remove.missing = FALSE), nominal.var.expected)
    expect_equal(SumForEachRow(variable.Nominal, remove.missing = TRUE), nominal.var.expected.wo.missing)
    ...
    expect_equal(SumForEachRow(variable.Binary, remove.missing = FALSE), binary.var.expected)
    expect_equal(SumForEachRow(variable.Binary, remove.missing = TRUE), binary.var.expected.wo.missing)
    expect_error(SumForEachRow(variable.Numeric, variable.Date),
                 paste0("Date/Time data has been supplied but ", sQuote("SumForEachRow"), " requires numeric data."))
    expect_error(SumForEachRow(variable.Binary, variable.Text),
                 paste0("Text data has been supplied but ", sQuote("SumForEachRow"), " requires numeric data."))
    ...
    expected.2.sum.rows.missing.kept <- array(rowSums(vars.as.matrix[, -3]),
                                              dim = c(length(variable.Binary), 1L),
                                              dimnames = list(NULL, "Coca-Cola + Age"))
    expect_equal(SumForEachRow(variable.Binary, variable.Numeric,
                               remove.missing = TRUE),
    ...
    expect_equal(SumForEachRow(variable.Binary, variable.Numeric, variable.Nominal,
                               remove.missing = TRUE),
    ...
    captured.warnings <- capture_warnings(SumForEachRow(variable.Binary,
                                                  variable.Nominal,
                                                  warn = TRUE))
    ...
    expect_warning(SumForEachRow(1:5, factor(1:5), warn = TRUE),
...
test_that("Table 1D", {
    expect_equivalent(SumForEachRow(table1D.Percentage), table1D.Percentage)
    expect_equal(SumForEachRow(table.1D.MultipleStatistics), rowSums(table.1D.MultipleStatistics))
    expected.output <- rowSums(table.1D.MultipleStatistics[, -which(colnames(table.1D.MultipleStatistics) == "z-Statistic")])
    expect_equal(SumForEachRow(table.1D.MultipleStatistics,
                               remove.columns = "z-Statistic"),
                 expected.output)
    captured.warnings <- capture_warnings(SumForEachRow(table.1D.MultipleStatistics,
                                                        remove.columns = "z-Statistic",
                                                        warn = TRUE))
    expect_setequal(captured.warnings,
                    paste0("The input data contains statistics of different types ",
                           ...
                           "Corrected p), it may not be appropriate to compute ", sQuote("SumForEachRow"), "."))
})
...
test_that("Table 2D", {
    ...
    expect_equal(SumForEachRow(table2D.Percentage), expected.2d.row.sums)
    expected.2d.row.sums <- rowSums(table2D.PercentageNaN[, -10], na.rm = TRUE)
    expect_equal(SumForEachRow(table2D.PercentageNaN), expected.2d.row.sums)
    ...
    expect_equal(SumForEachRow(table2D.PercentageAndCount),
                 row.summed.2d.table.multi.stats)
    # Warning about missing values
    expect_warning(SumForEachRow(table2D.PercentageNaN, warn = TRUE),
                   "Missing values have been ignored in calculation.")
    # Missing values
    expect_true(anyNA(SumForEachRow(table2D.PercentageNaN, remove.missing = FALSE)))
    expect_false(anyNA(SumForEachRow(table2D.PercentageNaN)))
    ...
    expect_equal(SumForEachRow(subsetted.qtable),
    ...
    captured.warnings <- capture_warnings(expect_equal(SumForEachRow(table.opp.inf, warn = TRUE),
                                                       expected.out))
    expect_setequal(captured.warnings,
                    c(paste0("The input data contains statistics of different types ",
                             ...
                             "appropriate to compute ", sQuote('SumForEachRow'), "."),
                      paste0(sQuote("SumForEachRow"),
    ...
    expect_setequal(captured.warnings,
                    c(paste0("The input data contains statistics of different types ",
                             ...
                             "d.f., z-Statistic, Corrected p), it may not be ",
                             "appropriate to compute ", sQuote('SumForEachRow'), "."),
                      paste0(sQuote("SumForEachRow"), " cannot be computed as the data contains both Inf and -Inf.")))
...
test_that("Higher dim Q tables", {
    ...
    expect_equal(SumForEachRow(curr.table),
...
test_that("Q Tables: Check warning of different statistics thrown or suppressed", {
    ...
    expect_equal(SumForEachRow(table.1D.MultipleStatistics), expected.out)
    warn.msg <- paste0("The input data contains statistics of different types ",
                       ...
                       "compute ", sQuote('SumForEachRow'), ".")
    captured.warnings <- capture_warnings(expect_equal(SumForEachRow(table.1D.MultipleStatistics, warn = TRUE),
                                                       expected.out))
    expect_setequal(captured.warnings, warn.msg)
    # No warning even if warn = TRUE when only a single statistic
    expect_equivalent(SumForEachRow(table1D.Average),
                      table1D.Average)
    expect_equivalent(SumForEachRow(table1D.Average, warn = TRUE),
                      table1D.Average)
    expected.out <- rowSums(table2D.Percentage)
    expect_equal(SumForEachRow(table2D.Percentage, remove.columns = NULL),
                 expected.out)
})
...
test_that("A single R Output (e.g. a vanilla matrix or vector) selected", {
    ...
    expect_equal(SumForEachRow(matrix.1), rowSums(matrix.1))
    vector.1 <-1:24
    expect_equal(SumForEachRow(vector.1), vector.1)
    # Don't support higher arrays
    array.1 <- array(1:504, dim = 7:9)
    expect_error(SumForEachRow(array.1),
                 paste0(sQuote("SumForEachRow"), " only supports inputs that have 1 or 2 dimensions. ",
...
test_that("Inappropriate multiple inputs", {
    expect_error(SumForEachRow(c(1:3), c(1:4)),
                 paste0(sQuote("SumForEachRow"), " requires all input elements to have the same ",
                        ...
                        "before attempting to call ", sQuote("SumForEachRow"), " again."),
                 fixed = TRUE)
    expect_error(SumForEachRow(c(1:3), c(1:4), c(1:10)),
                 paste0(sQuote("SumForEachRow"), " requires all input elements to have the same ",
                        ...
                        "before attempting to call ", sQuote("SumForEachRow"), " again."),
                 fixed = TRUE)
    expect_error(SumForEachRow(c(1:4), array(1:16, dim = c(4, 2, 2))),
                 paste0(sQuote("SumForEachRow"), " only supports inputs that have 1 or 2 dimensions. ",
                        "A supplied input has 3 dimensions. Contact support at ", contact.msg),
                 fixed = TRUE)
    expect_error(SumForEachRow(c(1:4), list("hello")),
                 paste0(sQuote("SumForEachRow"), " requires all input elements to be numeric vectors ",
    ...
    expect_error(SumForEachRow(1:nrow(table.1D.MultipleStatistics), table.1D.MultipleStatistics),
                 paste0(sQuote("SumForEachRow"), " doesn't support Tables when more than one input is provided. ",
                        "Either remove the input ", table.name, " and any other Tables from the ",
                        "input or call ", sQuote("SumForEachRow"), " again with only ", table.name, " ",
                        "as the input."),
                 fixed = TRUE)
})
...
test_that("Multiple inputs", {
    ...
    expect_equal(SumForEachRow(c(1:4), matrix(1:12, nrow = 4)), expected.output)
    expect_equal(SumForEachRow(c(A = 1, B = 2, C = 3, D = 4),
    ...
    expect_equal(SumForEachRow(vect, mat),
                 expected.with.names)
    dimnames(mat)[[1L]] <- letters[1:4]
    expect_equal(SumForEachRow(vect, mat), expected.with.names)
})
...
test_that("Column names conflicting with function argument names won't cause an error", {
    ...
    vec <- runif(10)
    expect_equal(SumForEachRow(input, vec), as.array(rowSums(cbind(input, vec))))
})
...
test_that("Relevant variable/variable sets identified", {
    ...
    sum.rows.output <- lapply(all.question.types, SumForEachRow)
